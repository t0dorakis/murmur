...
const spacedPoll = checkStatus().pipe(
...
```typescript
const daemon = processQueue().pipe(
...
const midnightJob = cleanup().pipe(
  Effect.repeat(Schedule.cron("0 0 * * *"))
)

// Run every Monday at 9 AM
const weeklyReport = generateReport().pipe(
  Effect.repeat(Schedule.cron("0 9 * * 1"))
)

// Run every 15 minutes
const frequentCheck = syncData().pipe(
...
const businessHours = task().pipe(
  Effect.repeat(
    Schedule.hourOfDay(9).pipe(
      Schedule.intersect(Schedule.dayOfWeek(1, 2, 3, 4, 5))
    )
  )
)
```
...
const program = Effect.gen(function* () {
  ...
  yield* breaker.call(externalApiCall()).pipe(
          ...
          return yield* effect.pipe(
            ...
            Effect.tapError(() =>
              Effect.gen(function* () {
                ...
                yield* Ref.set(openUntil, now + Duration.toMillis(Duration.decode(config.resetTimeout)))
              })
            )
          )
        })
...
const unionSchedule = Schedule.recurs(3).pipe(
...
const delayed = Schedule.recurs(5).pipe(
  Schedule.addDelay(() => "500 millis")
)

// Run while condition holds
const conditional = Schedule.forever.pipe(
  Schedule.whileInput<number>((n) => n < 100)
)

// Collect outputs
const collecting = Schedule.recurs(5).pipe(
