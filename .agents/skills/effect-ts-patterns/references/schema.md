...
const describe = (pet: Schema.Schema.Type<typeof Pet>) => {
  switch (pet._tag) {
    case "Dog": return `${pet.name} is a ${pet.breed}`
    case "Cat": return `${pet.name} is ${pet.indoor ? "indoor" : "outdoor"}`
  }
}
```
...
const Employee = Schema.extend(Person, Schema.Struct({
...
const result = yield* parseUser(data).pipe(
