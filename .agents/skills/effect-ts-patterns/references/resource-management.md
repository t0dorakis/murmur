...
const program = Effect.scoped(
  withResources.pipe(
    Effect.flatMap(([db, cache, queue]) => doWork(db, cache, queue))
  )
)
```
...
const InfrastructureLive = Layer.merge(
  DatabaseLive,
  Layer.merge(CacheLive, QueueLive)
)
...
const streamFromResource = <A, R>(
  ...
  Stream.unwrapScoped(
    Effect.acquireRelease(acquire, release).pipe(
      Effect.map((resource) =>
        Stream.repeatEffectOption(produce(resource))
      )
    )
  )
