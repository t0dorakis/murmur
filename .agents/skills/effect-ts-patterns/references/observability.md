...
const processWithTiming = Effect.gen(function* () {
  yield* Effect.logSpan("database-query")(
    databaseQuery()
  )

  yield* Effect.logSpan("external-api-call")(
    callExternalApi()
  )

  yield* Effect.logSpan("data-transformation")(
    transformData()
  )
})
```
...
const program = myEffect.pipe(
  Effect.provide(verboseLogging(process.env.DEBUG === "true"))
)
```
...
const processRequest = (requestId: string) =>
  Effect.gen(function* () {
    ...
    const permissions = yield* checkPermissions(user).pipe(
      Effect.withSpan("checkPermissions")
    )

    const result = yield* executeAction(user, permissions).pipe(
      Effect.withSpan("executeAction")
    )
...
const trackRequestSize = (bytes: number) =>
  Metric.update(requestSize, bytes)
```
...
const metricsEndpoint = HttpRouter.get("/metrics",
  Effect.gen(function* () {
...
const healthCheck = Effect.gen(function* () {
  ...
  yield* Metric.set(
    Metric.gauge("health_check_status"),
    healthy ? 1 : 0
  )
