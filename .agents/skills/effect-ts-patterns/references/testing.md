...
const MockUserRepository = (users: Map<string, User> = new Map()) =>
  Layer.succeed(UserRepository, {
    findById: (id) =>
      Effect.sync(() => Option.fromNullable(users.get(id))),
    create: (user) =>
      Effect.sync(() => {
...
describe("deleteUser", () => {
  it("should delete existing user", async () => {
    ...
    const result = await Effect.runPromise(
      deleteUser("1").pipe(
        Effect.provide(MockUserRepository(users))
      )
    )
...
it("should call repository methods", async () => {
  ...
  await Effect.runPromise(
    createUser({ name: "Alice" }).pipe(Effect.provide(service))
  )
...
it("should fail with NotFoundError", async () => {
  const exit = await Effect.runPromiseExit(
    getUser("nonexistent").pipe(Effect.provide(TestLayer))
  )
...
it("should handle typed errors", async () => {
  const effect: Effect.Effect<User, NotFoundError | ValidationError> =
    getUser("invalid")
  ...
  Exit.match(exit, {
    onFailure: (cause) => {
      ...
      expect(Chunk.toArray(failures)).toEqual([
        expect.objectContaining({ _tag: "NotFound" })
      ])
    },
    onSuccess: () => {
...
it("should handle stream errors", async () => {
  const stream = Stream.fromIterable([1, 2, 3]).pipe(
    Stream.mapEffect((n) =>
      n === 2 ? Effect.fail(new ProcessingError()) : Effect.succeed(n)
    )
  )
    ...
    fc.property(
      ...
      async (user) => {
        ...
        const decoded = await Effect.runPromise(
          Schema.decodeUnknown(User)(encoded)
        )
...
it("should reject invalid emails", () => {
  ...
  fc.assert(
    fc.property(
      ...
      async (invalidEmail) => {
        const exit = await Effect.runPromiseExit(
          Schema.decodeUnknown(Email)(invalidEmail)
        )
